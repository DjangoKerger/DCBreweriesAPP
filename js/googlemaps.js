...
searchByLocation().then((data) => {
  ...
  makeMarker(
    ...
    data.coord.lng,
    "myLocation",
    "https://img.icons8.com/officexs/2x/dizzy-person.png"
  );

  for (let index = 0; index < hundredMileRadiusData.length; index++) {
    let title = hundredMileRadiusData[index].brewery.name;
    makeMarker(
      hundredMileRadiusData[index].latitude,
      hundredMileRadiusData[index].longitude,
      title,
      "https://img.icons8.com/officexs/2x/beer.png"
    ); // icon
  }

  renderList(hundredMileRadiusData);
});
function makeMarker(lat, lng, title, iconUrl) {
  //icon
